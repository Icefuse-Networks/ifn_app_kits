    ...
    return (
      <div className="flex-1 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-zinc-500 text-sm">Loading mappings...</p>
        </div>
      </div>
    )
  }
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="p-4 border-b border-white/10 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-zinc-400">Assign kit configs to servers</p>
        </div>
      </div>

      {/* Messages */}
      {error && (
        <div className="mx-4 mt-4 flex items-center gap-2 px-3 py-2 bg-red-500/10 border border-red-500/20 rounded-lg text-sm text-red-400">
          <AlertCircle className="h-4 w-4 shrink-0" />
          {error}
        </div>
      )}
      {success && (
        <div className="mx-4 mt-4 flex items-center gap-2 px-3 py-2 bg-green-500/10 border border-green-500/20 rounded-lg text-sm text-green-400">
      ...
      <div className="p-4 border-b border-white/10">
        <div className="grid grid-cols-2 gap-3">
          {/* Kit Config Dropdown */}
          <div>
            <label className="block text-xs text-zinc-400 mb-1.5">Kit Config</label>
            <select
              value={selectedConfig || ''}
              onChange={(e) => setSelectedConfig(e.target.value || null)}
              className="w-full px-3 py-2 rounded-lg text-sm text-white bg-zinc-800 border border-white/10 focus:outline-none focus:ring-2 focus:ring-purple-500"
            >
          ...
          {/* Server Multi-Select */}
          <div>
            <label className="block text-xs text-zinc-400 mb-1.5">Servers</label>
            <MultiSelect
        ...
        <div className="mt-3 flex items-center justify-between">
          ...
          <button
            ...
            className="ml-auto flex items-center gap-2 px-4 py-2 bg-purple-500 text-white rounded-lg text-sm font-medium hover:bg-purple-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
