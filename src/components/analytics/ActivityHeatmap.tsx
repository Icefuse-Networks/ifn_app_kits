      ...
      <div className="min-w-[700px]">
        <div className="flex mb-2">
          ...
          {Array(24)
            .fill(null)
            .map((_, h) => (
              <div key={h} className="flex-1 text-center text-xs text-zinc-500">
                {h}
              </div>
            ))}
        </div>
        {data.map((row, dayIdx) => (
          <div key={dayIdx} className="flex items-center gap-1 mb-1">
            <div className="w-12 text-sm text-zinc-400 font-medium">{DAYS[dayIdx]}</div>
