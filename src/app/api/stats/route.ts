...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const cacheKey = `${view}:${table}:${server_id}:${search || ''}:${safeSort}:${safeOrder}:${limit}:${offset}`
    ...
    if (view === 'clans') {
      return handleClansQuery(table, server_id, search, safeSort, safeOrder, limit, offset, cacheKey)
    }

    return handlePlayersQuery(table, server_id, search, safeSort, safeOrder, limit, offset, cacheKey)
  } catch (error) {
...
async function handleClansQuery(
  ...
  // Config-driven clan SELECT columns
  const dataResult = await clickhouse.query({
    query: `
      SELECT
        ${buildClanSelectColumns()}
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    if (!parsed.success) {
      return NextResponse.json(
        { success: false, error: { code: 'VALIDATION_ERROR', message: 'Invalid wipe parameters', details: parsed.error.flatten() } },
        { status: 400 }
      )
    }

    const { server_id, scope } = parsed.data
    const table = TIMEFRAME_TABLE[scope]
    ...
    await createAuditLog({
      action: 'delete',
      resourceType: 'stats_wipe',
      resourceId: server_id,
      authContext: authResult.context,
      oldValues: { scope, table },
      request,
    })

    logger.stats.info('Stats wiped', {
      server: server_id,
      scope,
