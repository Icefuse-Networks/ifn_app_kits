...
// SECURITY: Zod validated schemas
...
const updateMappingSchema = z.object({
  ...
  minutesAfterWipe: z.number().int().min(0).nullable().optional(),
  configId: z.string().min(1).max(60).optional(),
});
...
export async function GET(request: NextRequest) {
  ...
  try {
    const mappings = await prisma.kitMapping.findMany({
      include: {
    ...
    console.error("Error fetching kit mappings:", error);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const mapping = await prisma.kitMapping.create({
    ...
    console.error("Error creating kit mapping:", error);
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    await prisma.kitMapping.delete({ where: { id: parsed.data.id } });
    return NextResponse.json({ success: true });
  } catch (error) {
    console.error("Error deleting kit mapping:", error);
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    const data: { isLive?: boolean; minutesAfterWipe?: number | null; configId?: string } = {};
    ...
    const mapping = await prisma.kitMapping.update({
      where: { id },
      data,
      include: {
        config: { select: { id: true, name: true } },
    ...
    console.error("Error updating kit mapping:", error);
