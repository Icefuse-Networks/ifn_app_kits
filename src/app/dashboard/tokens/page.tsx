 ...
 *
 * Allows admins to create, view, and revoke API tokens.
 * IMPORTANT: Full token is only shown once on creation.
 */

import { useState, useEffect, useCallback } from 'react'
import { useRouter } from 'next/navigation'
import { Key, Plus, Trash2, Copy, Check, AlertTriangle, Clock, Shield, X, ArrowLeft } from 'lucide-react'
import type { ApiTokenInfo, ApiScope, ApiTokenCreateResponse } from '@/types/api'
...
export default function TokensPage() {
  ...
  const handleCreate = async () => {
    ...
    setCreating(true)
    try {
      const body: { name: string; scopes: string[]; expiresAt?: string } = {
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 bg-[var(--accent-primary)] hover:bg-[var(--accent-secondary)] text-white rounded-[var(--radius-md)] transition-colors"
        >
      ...
      {/* Error message */}
      {error && (
        <div className="mb-6 p-4 bg-[var(--status-error)]/10 border border-[var(--status-error)]/30 rounded-[var(--radius-md)] flex items-center gap-3">
      ...
      {newToken && (
        ...
        >
          ...
          >
            ...
            <div className="p-4 bg-[var(--status-warning)]/10 border border-[var(--status-warning)]/30 rounded-[var(--radius-md)] mb-4">
              ...
              <div className="flex items-center gap-2">
                <code className="flex-1 p-3 bg-[var(--bg-input)] rounded-[var(--radius-sm)] text-sm font-mono text-[var(--text-primary)] break-all">
                  {newToken.token}
                </code>
                <button
                  onClick={copyToken}
                  className="p-2 bg-[var(--accent-primary)] hover:bg-[var(--accent-secondary)] text-white rounded-[var(--radius-sm)] transition-colors"
                >
            ...
            <div className="p-4 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 bg-[var(--bg-input)] border border-[var(--border-secondary)] rounded-[var(--radius-md)] text-[var(--text-primary)] placeholder:text-[var(--text-muted)] focus:border-[var(--accent-primary)] focus:outline-none"
                />
              </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {API_SCOPES.map((scope) => (
                    <label
                      key={scope}
                      className="flex items-center gap-3 p-3 bg-[var(--glass-bg)] border border-[var(--glass-border)] rounded-[var(--radius-md)] cursor-pointer hover:bg-[var(--glass-bg-prominent)] transition-colors"
                    >
                      <input
                        type="checkbox"
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 bg-[var(--bg-input)] border border-[var(--border-secondary)] rounded-[var(--radius-md)] text-[var(--text-primary)] focus:border-[var(--accent-primary)] focus:outline-none"
                />
              </div>
            </div>
      ...
      {loading ? (
        <div className="text-center py-12">
          <div className="inline-block w-8 h-8 border-2 border-[var(--accent-primary)] border-t-transparent rounded-full animate-spin"></div>
          <p className="mt-4 text-[var(--text-muted)]">Loading tokens...</p>
        </div>
      ) : tokens.length === 0 ? (
        <div className="text-center py-12 bg-[var(--glass-bg)] border border-[var(--glass-border)] rounded-[var(--radius-lg)]">
          ...
          <button
            onClick={() => setShowCreateModal(true)}
            className="inline-flex items-center gap-2 px-4 py-2 bg-[var(--accent-primary)] hover:bg-[var(--accent-secondary)] text-white rounded-[var(--radius-md)] transition-colors"
          >
            <Plus className="w-4 h-4" />
            Create Token
          </button>
        ...
        <div className="space-y-4">
          {tokens.map((token) => {
            ...
            return (
              <div
                key={token.id}
                className={`p-4 bg-[var(--glass-bg)] border border-[var(--glass-border)] rounded-[var(--radius-lg)] ${
                ...
                <div className="flex items-start justify-between">
                  ...
                  {!token.isRevoked && (
                    <button
                      ...
                      className="p-2 text-[var(--status-error)] hover:bg-[var(--status-error)]/10 rounded-[var(--radius-md)] transition-colors disabled:opacity-50"
                      title="Revoke token"
                    >
