...
interface AnalyticsData {
  overview: {
    totalEvents: number; uniqueWinners: number; avgParticipants: number; totalKills: number;
    avgDuration: number; kothCount: number; mazeCount: number;
  };
...
export default function EventsTab({ timeFilter, serverFilter, servers, onServersFound, serverNameMap }: EventsTabProps) {
  ...
  const [activeSubTab, setActiveSubTab] = useState<'analytics' | 'events'>('analytics');
  ...
  }, [currentPage, serverFilter, eventTypeFilter, timeFilter, onServersFound]);
  ...
  const fetchAnalytics = useCallback(async () => {
    ...
    try {
      ...
      eventsList.forEach(e => {
        ...
        existing.wins++; existing.kills += e.winner_kills; existing.events++;
        winnerMap.set(e.winner_steam_id, existing);
      });
      const topWinners = Array.from(winnerMap.entries()).map(([steam_id, stats]) => ({ steam_id, ...stats })).sort((a, b) => b.wins - a.wins).slice(0, 10);

      const modeMap = new Map<string, number>();
      eventsList.forEach(e => {
        e.event_modes?.forEach(mode => modeMap.set(mode, (modeMap.get(mode) || 0) + 1));
        if (!e.event_modes?.length) modeMap.set('Standard', (modeMap.get('Standard') || 0) + 1);
      });
      const eventModes = Array.from(modeMap.entries()).map(([mode, count]) => ({ mode, count })).sort((a, b) => b.count - a.count);
  ...
  const formatDuration = (seconds: number) => { const mins = Math.floor(seconds / 60); return `${mins}m ${seconds % 60}s`; };
    ...
    <>
      <div className="flex gap-3 flex-wrap mb-4">
        <Tabs
          ...
          onChange={(tab) => setActiveSubTab(tab as 'analytics' | 'events')}
          variant="pills" size="sm"
        />
        <Dropdown value={eventTypeFilter} options={[
          { value: '', label: 'All Events' }, { value: 'koth', label: 'KOTH' }, { value: 'maze', label: 'Maze' }
        ]} onChange={(val) => { setEventTypeFilter(val as "" | "koth" | "maze"); setCurrentPage(1); }} />
        <Button onClick={() => { fetchEvents(); fetchAnalytics(); }} disabled={loading || analyticsLoading} loading={loading || analyticsLoading} icon={<RefreshCw className="h-4 w-4" />} variant="primary">Refresh</Button>
      </div>
      ...
      {activeSubTab === 'analytics' && (
        <>
            ...
            <>
              ...
              <ChartCard title="Events & Activity Over Time" icon={TrendingUp} className="lg:col-span-2" delay={0.4}>
                <div className="h-80">
                  ...
                  ]} showDualYAxis yAxisNames={["Count", "Kills"]} height="100%" />
                </div>
              </ChartCard>

              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <ChartCard title="Top Winners" icon={Crown} delay={0.45}>
                  <div className="h-72"><BarChart data={analytics.topWinners.slice(0, 8).map(w => ({ label: w.name, value: w.wins }))} height="100%" colors={COLORS} /></div>
                </ChartCard>
          ...
          ) : <EmptyState icon={<Trophy className="h-12 w-12" />} title="No event data available" />}
        </>
      )}
