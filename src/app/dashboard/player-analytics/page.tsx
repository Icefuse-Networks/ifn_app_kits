      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
          ...
          <ChartCard title="Server Comparison" icon={Server} iconColor="text-blue-400" delay={0.3}>
            <div className="h-72">
              {serverComparisonData.data.length > 0 ? (
                <TimeSeriesChart
                  data={serverComparisonData.data}
                  series={serverComparisonData.series}
                  height="100%"
                />
              ) : (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          <ChartCard title="Server Utilization" icon={BarChart3} iconColor="text-green-400" delay={0.4}>
            <div className="h-64">
              <BarChart data={utilizationData} height="100%" colors={COLORS} labelWidth={80} valueFormatter={(v) => `${v.toFixed(1)}%`} />
            </div>
          </ChartCard>

          <ChartCard title="Peak Players by Server" icon={Zap} iconColor="text-yellow-400" delay={0.5}>
            <div className="h-64">
              <PieChart data={peakPlayersData} height="100%" colors={COLORS} />
            </div>
          </ChartCard>

          <ChartCard title="Activity Heatmap" icon={Clock} iconColor="text-pink-400" delay={0.6}>
            <div className="h-64 overflow-auto">
              <ActivityHeatmap data={heatmapGrid} tooltipPrefix="players" cellHeight={20} />
            </div>
          </ChartCard>
        </div>
        ...
        <ChartCard title="Server Statistics" icon={Server} iconColor="text-purple-400" delay={0.7}>
          {loading ? (
            ...
            <DataTable
              ...
              emptyIcon={<Server className="h-12 w-12 mx-auto mb-3 opacity-30" />}
            />
          )}
        </ChartCard>
      </motion.div>
